...
def list_tasks():
    ...
    if not tasks:
        print("Não possui tarefas disponíveis!")
        return False
    else:
        for task in tasks:
            print(f"ID: {task[0]}, Descrição: {task[1]}, concluída: {'Sim' if task[2] else 'Não'}")
...
""")
    ...
    match decisao:
        ...
        case 3:
            if list_tasks():
                while True:
                    choose_task_to_complete = int(input("Qual o id da tarefa que deseja marcar como Concluída? "))
        ...
        case 4:
            if list_tasks():
                while True:
                    choose_task_to_delete = int(input("Digite o id da tarefa que deseja deletar: "))
